void IFFT_64pt(int *x_re, int *x_im, int *out_re, int *out_im)
{
	...
	{
		input[data].re = x_im[data];
		input[data].im = x_re[data];
	}
	...
	for (n = 0; n<N; n++)
	{
		...
		out_im[n] = (X[n].re) >> (6 + TX_IFFT_OUT_SCALE_DN);
		out_re[n] = (X[n].im) >> (6 + TX_IFFT_OUT_SCALE_DN);
	}
