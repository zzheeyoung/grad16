...
//snr power range
#define snr_init 0
#define snr_range 10//dB  15
#define snr_between 1
#define loop_range 1000//Number of PPDU
...
Vit_Dec_Soft<int, int> vit_dec_soft;
...
void main()
{
	...
	*/
	...
	float *flt_received_I, *flt_received_Q;
	...
	flt_received_I = new float[length_ppdu];
	flt_received_Q = new float[length_ppdu];
	...
	for (MCS = 4; MCS < 5; MCS++)
	{
		...
		for (n = 0; n < snr_range; n++)
		{
			...
			for (int loop = 0; loop < loop_range; loop++)
			{
				...
				TextOutput(&data_re[0], "..\\..\\data_re.txt",64*4);
				/////////////////////////
				/* STF Generation */
				/////////////////////////
				...
				TextOutput(&ppdu_re[0], "..\\..\\ppdu_re.txt",320);
				////////////////////////////
				/* Data IFFT Process */
				////////////////////////////
				//cout<<"Data : "<<endl;
				...
				TextOutput(&ppdu_re[0], "..\\..\\ifftppdu_re.txt",400);
				//if (snr == 10)
				...
				for (k = 0; k<length_ppdu; k++) {
					flt_received_I[k] += fading_I[k];
					flt_received_Q[k] += fading_Q[k];
				}


				for (k = 0; k<length_ppdu; k++) {
					// pass channel / no doppler / Rayleigh fading
					rx_no_awgn_I[k + T_delay] = (int)(pow(2, sc_up)*flt_received_I[k]);
					rx_no_awgn_Q[k + T_delay] = (int)(pow(2, sc_up)*flt_received_Q[k]);
				}

				//Noise
				AwgnGen(flt_received_I, flt_received_I, length_ppdu, 1 / sqrt(128));
				AwgnGen(flt_received_Q, flt_received_Q, length_ppdu, 1 / sqrt(128));
				...
				for (i = 0; i < length_ppdu; i++)
				{
					received_I[i] = (int)(pow(2, sc_up)*flt_received_I[i]);
					received_Q[i] = (int)(pow(2, sc_up)*flt_received_Q[i]);
				}
				TextOutput(&flt_received_I[0], "..\\..\\flt_received_I.txt",320);
				//if (snr == 10)
				//	TextOutput(&received_I[0], "..\\..\\stfltf.txt",320);
				//fft 
				TextOutput_(&received_I[0], "..\\..\\received_I.txt",320);
				...
				TextOutput_(&rx_freq_re[0], "..\\..\\rx_freq_re.txt",320);
				// channel estimation
				for (j = 0; j<64; j++)
				{
				...
				TextOutput_(&h_re[0], "..\\..\\h_re.txt",64);
				for (i = 0; i<module_param[3] + 1; i++)
				{
					for (k = 0; k<64; k++)
					{
				...
				TextOutput_(&data_out_re[0], "..\\..\\data_out_re.txt",64*4);
				frame_inf = 0;
				tmp_frane = 0;
				frame_length = 0;
				toggle = 0;
				...
				for (k = 0; k<2; k++)
				{
					...
					else
					{
						//Soft Demapping
						DeMapper(data_out_re, data_out_im, bit_out, module_param, h_re, h_im, tx_signal_scaling, toggle, frame_inf);
						//(-2147483648 <= n && n <= 2147483647) int 형 범위
						TextOutput_(&bit_out[0], "..\\..\\bit_out.txt", 48*10);
						//Deinterleaving				
						deinterleaving(bit_out, bit_out_int, module_param, toggle, frame_inf);
				...
				TextOutput_(&bit_out[0], "..\\..\\bit_out.txt", 64 * 4);
				////////////////////////////
				// Data - Descrmabler
				////////////////////////////
				if (Parity_check == 1)
