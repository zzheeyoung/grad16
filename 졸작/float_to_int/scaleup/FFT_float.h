...
public:
 ...
 private:
    ...
    void reordering_20();
	void proc_fft_20(T *in_re, T *in_im, T *out_re, T *out_im);
};
...
void FFT<T>::proc_FFT() {
	...
	for (i=0;i<num_sym/64;i++) {
		...
		proc_fft_20(in_re_tmp, in_im_tmp, out_re_tmp, out_im_tmp);
		for (j=0;j<64;j++) {
			out_I[i*64+j] = out_re_tmp[j];
			out_Q[i*64+j] = out_im_tmp[j];
		}
	}
	delete []in_re_tmp;
	delete []in_im_tmp;
...
template <class T>
void FFT<T>::proc_fft_20(T *in_re, T *in_im, T *out_re, T *out_im){
...
template <class T>
void FFT<T>::butterfly1_20(unsigned char i)
{
...
template <class T>
void FFT<T>::butterfly2_20(unsigned char i)
{
...
template <class T>
void FFT<T>::butterfly3_20(unsigned char i)
{
...
template <class T>
void FFT<T>::multiply1_20(unsigned char i)
{
...
template <class T>
void FFT<T>::multiply2_20(unsigned char i)
{
  in_Idata[i] = out_Idata[i];
  in_Qdata[i] = out_Qdata[i];
